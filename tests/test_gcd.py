...
def test_gcd():
    ...
    assert gcd(56, 98) == 14
    assert gcd(101, 10) == 1  
    assert gcd(0, 5) == 5      # a = 0
    assert gcd(5, 0) == 5     # b = 0

    # Test case 2: Large numbers
    assert  gcd(12345678901234567890, 98765432109876543210) == 900000000090
    assert  gcd(12345678901234567890, 12345678901234567890) == 12345678901234567890
    assert  gcd(1000000000000000000000, 1000000000000000000001) == 1  
    assert  gcd(99999999999999999999, 99999999999999999998) == 1   
    assert  gcd(1000000000000000000001, 1000000000000000000003) == 1  
    assert  gcd(10**18, 10**9) == 10**9   
    assert  gcd(10**40, 10**20) == 10**20  
    assert  gcd(9999999967, 9999999969) == 1 
    assert  gcd(982451653, 982451653) == 982451653  
    assert  gcd(982451653, 57885161) == 1 
    assert  gcd(32452843, 982451653) == 1  
    assert  gcd(2147483647, 2305843009213693951) == 1   

def test_ex_gcd():
    # Test case 1: Small numbers
    gcd_val, x, y =  ex_gcd(48, 18)
    assert gcd_val == 6 and 48 * x + 18 * y == gcd_val, f"Expected GCD(48, 18) = 6, coefficients {x}, {y}"
    
    gcd_val, x, y =  ex_gcd(101, 10)
    assert gcd_val == 1 and 101 * x + 10 * y == gcd_val, f"Expected GCD(101, 10) = 1, coefficients {x}, {y}"
    
    gcd_val, x, y =  ex_gcd(0, 5)  # a = 0
    assert gcd_val == 5 and 0 * x + 5 * y == gcd_val, f"Expected GCD(0, 5) = 5, coefficients {x}, {y}"
    
    gcd_val, x, y =  ex_gcd(5, 0)  # b = 0
    assert gcd_val == 5 and 5 * x + 0 * y == gcd_val, f"Expected GCD(5, 0) = 5, coefficients {x}, {y}"

    # Test case 2: Large numbers
    gcd_val, x, y =  ex_gcd(1000000000000000000000, 1000000000000000000001)
